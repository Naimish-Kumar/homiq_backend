...
class NeighborhoodInsightController extends Controller
{
    ...
    {
        $data = Cache::remember("property_insights_{$id}", 86400, function () use ($id) {
            ...
            $placeTypes = [
                ...
                'transit_station' => ['label' => 'Transit', 'fallback' => ['Subway Station', 'Metro Station', 'Bus Stop']],
                'supermarket' => ['label' => 'Grocery', 'fallback' => ['Organic Foods', 'City Supermarket', 'Fresh Mart', 'Daily Needs']],
                'hospital' => ['label' => 'Health', 'fallback' => ['City Hospital', 'General Medical Clinic', 'Community Pharmacy']],
                'school' => ['label' => 'Education', 'fallback' => ['High School', 'Community College', 'Primary School']],
            ];
            ...
            foreach ($placeTypes as $type => $info) {
                ...
                if ($nearest) {
                    ...
                    $distanceVal = rand(100, 1200);
                    $places[] = [
                        'name' => $fallbackNames[rand(0, count($fallbackNames) - 1)],
                        'type' => $info['label'],
                        'distance' => $distanceVal . 'm',
                        'lat' => $lat + (rand(-100, 100) / 100000.0),
                        'lng' => $lng + (rand(-100, 100) / 100000.0),
                    ];
                }
            }
